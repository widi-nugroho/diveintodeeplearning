...
class Mlp_dropout_scratch {
    ...
    var dropout:Float=0f
    var numInputs:Long=0
    var numHiddens:Long=0
    var numOutputs:Long=0
    val loss: Loss = Loss.softmaxCrossEntropyLoss()

    constructor(manager: NDManager, numInputs:Long, numHiddens:Long, numOutputs:Long,dropout: Float){
        ...
        w1 = this.manager.randomNormal(0f, 0.01f, Shape(numInputs, numHiddens), DataType.FLOAT32, Device.defaultDevice())
        b1 = this.manager.zeros(Shape(numHiddens))
        w2 = this.manager.randomNormal(0f, 0.01f, Shape(numHiddens, numOutputs), DataType.FLOAT32, Device.defaultDevice())
        b2 = this.manager.zeros(Shape(numOutputs))
    ...
    fun net(X: NDArray): NDArray {
        ...
        val HD = dropoutLayer(H,dropout,manager)
        return HD.dot(w2).add(b2)
    }
    fun dropoutLayer(x:NDArray,dropout: Float,manager:NDManager): NDArray {
        if (dropout == 1.0f){
            return  manager.zeros(x.shape)
        }else if (dropout == 0f){
            return x
        }else{
            var mask = manager.randomUniform(0f,1.0f,x.getShape()).gt(dropout)
...
fun main(){
    ...
    val k = Mlp_dropout_scratch(manager,numInputs,numHiddens,numOutputs,dropout)
